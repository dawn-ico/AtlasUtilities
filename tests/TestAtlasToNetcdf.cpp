...
int main(int argc, char const* argv[]) {
  ...
  // version that uses atlas to compute additional neighbor lists
  {
    ...
    // round trip
    AtlasToNetCDF(meshOpt.value(), outFname);
    meshOpt = AtlasMeshFromNetCDFMinimal(outFname);
    ...
    // dump using the atlas gmsh writer
  ...
  // version that reads additional neighbor lists from the netcdf file
  {
    ...
    // round trip
    AtlasToNetCDF(meshOpt.value(), outFname);
    meshOpt = AtlasMeshFromNetCDFComplete(outFname);
    auto mesh = meshOpt.value();
