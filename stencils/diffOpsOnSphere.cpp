...
const double RADIUS_SPHERE = 1; // has to be = 1 for this file (not considering R in del
                                // operator for reference solutions!)
...
std::tuple<double, double> cartToLongLat(double x, double y, double z) {
  double lambda = atan2(y, x);
  double phi = asin(z / RADIUS_SPHERE);
  return {lambda, phi};
}
...
void gradient(const atlas::Mesh& mesh, const atlasInterface::Field<double>& u,
              atlasInterface::SparseDimension<double>& nxLCce,
              atlasInterface::SparseDimension<double>& nyLCce,
              ...
              atlasInterface::Field<double>& u_lambda, atlasInterface::Field<double>& u_phi,
              int level);

void divergence(const atlas::Mesh& mesh, const atlasInterface::Field<double>& uvn,
                atlasInterface::SparseDimension<double>& edge_orientation_cell,
                const atlasInterface::Field<double>& L, const atlasInterface::Field<double>& A,
                atlasInterface::Field<double>& div, int level);

void curl(const atlas::Mesh& mesh, const atlasInterface::Field<double>& uvn,
          atlasInterface::SparseDimension<double>& edge_orientation_vertex,
          const atlasInterface::Field<double>& dualL, const atlasInterface::Field<double>& dualA,
          atlasInterface::Field<double>& rot, int level);
...
int main(int argc, char const* argv[]) {
  ...
  // dump a whole bunch of debug output
  const bool dbg_out = true;

  // this is meant to be run on a (spherical) netcdf mesh
  ...
  // atlas is not computing node to cell lists, so lets add them ourselves
  ...
  // netcdf mesh has lon/lat set in degrees but we want radians
  // and we also want cartesian coordinates
  std::vector<glm::dvec3> xyz(mesh.nodes().size());
  {
    ...
    for(int nodeIdx = 0; nodeIdx < mesh.nodes().size(); nodeIdx++) {
      ...
      auto [lonRet, latRet] = cartToLongLat(x, y, z);
  ...
  // dump mesh (vis e.g. in octave)
  if(dbg_out) {
    dumpMesh4Triplot(mesh, "diffOpsMesh", xyz);
  }
  ...
  auto [curl_F, curlField] = MakeAtlasField("curl", mesh.nodes().size());
  ...
  // However, ALSO NOTE that phi in geoscience is phase shifted by PI/2!!!!1
  ...
  for(int edgeIdx = 0; edgeIdx < mesh.edges().size(); edgeIdx++) {
    ...
    auto [lambda, phi] = cartToLongLat(m.x, m.y, m.z);
    auto [uIdx, vIdx] = sphericalHarmonic(lambda, phi);
  ...
  for(int cellIdx = 0; cellIdx < mesh->cells().size(); cellIdx++) {
    ...
    auto [lambda, phi] = cartToLongLat(m.x, m.y, m.z);
    divSol(cellIdx, level) = analyticalDivergence(lambda, phi);
    auto [gradUx, gradUy] = analyticalGradientU(lambda, phi);
    auto [gradVx, gradVy] = analyticalGradientV(lambda, phi);
  ...
  for(int nodeIdx = 0; nodeIdx < mesh->nodes().size(); nodeIdx++) {
    auto m = xyz[nodeIdx];
    auto [lambda, phi] = cartToLongLat(m.x, m.y, m.z);
    curlSol(nodeIdx, level) = analyticalCurl(lambda, phi);
  }

  //===------------------------------------------------------------------------------------------===//
  // gradients
  //===------------------------------------------------------------------------------------------===//
  gradient(mesh, u, nxLCce, nyLCce, edge_orientation_cell, L, A, grad_u_x, grad_u_y, level);
  gradient(mesh, v, nxLCce, nyLCce, edge_orientation_cell, L, A, grad_v_x, grad_v_y, level);

  //===------------------------------------------------------------------------------------------===//
  // divergence
  //===------------------------------------------------------------------------------------------===//
  divergence(mesh, uvn, edge_orientation_cell, L, A, div, level);

  //===------------------------------------------------------------------------------------------===//
  // curl
  //===------------------------------------------------------------------------------------------===//
  curl(mesh, uvn, edge_orientation_vertex, dualL, dualA, curlField, level);

  // dump a csv with results and references, vis. w/ paraview or octave
  if(dbg_out) {
    const auto& conn = mesh->cells().edge_connectivity();
    const auto& connNodes = mesh->cells().node_connectivity();
    FILE* fp = fopen("diffOps.csv", "w+");
    ...
    for(int cellIdx = 0; cellIdx < mesh.cells().size(); cellIdx++) {
      ...
      for(int nbhIdx = 0; nbhIdx < 3; nbhIdx++) {
        curlIdx += curlField(connNodes(cellIdx, nbhIdx), level);
  ...
  {
    auto [Linf, L1, L2] = MeasureErrors(mesh->nodes().size(), curlSol, curlField, level);
    printf("curl: %e %e %e\n", Linf, L1, L2);
  }
}
...
glm::dvec2 myprojLTPC_ENU(const glm::dvec3& pCart, const glm::dvec3& pRefCart) {
  auto [lambda, phi] = cartToLongLat(pRefCart.x, pRefCart.y, pRefCart.z);
...
void gradient(const atlas::Mesh& mesh, const atlasInterface::Field<double>& u,
  ...
  for(int cellIdx = 0; cellIdx < mesh.cells().size(); cellIdx++) {
    ...
    u_lambda(cellIdx, level) = lhs_x / A(cellIdx, level);
    u_phi(cellIdx, level) = lhs_y / A(cellIdx, level);
  }
}
...
void curl(const atlas::Mesh& mesh, const atlasInterface::Field<double>& uvn,
  ...
  for(int nodeIdx = 0; nodeIdx < mesh.nodes().size(); nodeIdx++) {
    ...
    rot(nodeIdx, level) = lhs / dualA(nodeIdx, level);
  }
}